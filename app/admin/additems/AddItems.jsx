...
const AddItems = () => {
  ...
  const handleFileChange = (e, imageField) => {
    ...
    if (file) {
      ...
      setValue(imageField, file); // Store in form
  ...
  const onSubmit = async (data) => {
    ...
    Object.keys(data).forEach((key) => {
      if (data[key] instanceof File) {
        formData.append(key, data[key], `${Date.now()}-${data[key].name}`); // Unique filename
      } else {
        ...
        {[1, 2, 3, 4, 5, 6].map((num) => (
          <div key={num} className="flex flex-col items-center">
            ...
            {previewImages[`image${num}`] && (
              <img src={previewImages[`image${num}`]} alt={`Preview ${num}`} className="w-20 h-20 object-cover mt-2" />
            )}
          </div>
        ))}
